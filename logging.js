...
(function() {
    ...
    function logEvent(event, customName, customInfo) {
        ...
        var ct = customInfo.ct;
        var block_n = customInfo.block_n;
        var trial_n = customInfo.trial_n;
        var width = customInfo.width;
        var eww_ratio = customInfo.eww_ratio;
        var density = customInfo.density;
        var amplitude = customInfo.amplitude;
        var movement_time = customInfo.movement_time;
        var errors = customInfo.errors;

        if (ENABLE_CONSOLE_LOGGING) {
            console.log(uid, ct, block_n, trial_n, width, eww_ratio, density, amplitude, movement_time, errors);
        }
        if (ENABLE_NETWORK_LOGGING) {
            sendNetworkLog(uid, ct, block_n, trial_n, width, eww_ratio, density, amplitude, movement_time, errors);
        }
    }
...
// docs.google.com/forms/d/e/1FAIpQLSe243u0aoo6u-0xw5nrunokJyXPq1nIM8qd5VnE_92fJISzDA/viewform?usp=sf_link
function sendNetworkLog(
    ...
    movement_time,
    errors) {
    var formid = "e/1FAIpQLSe243u0aoo6u-0xw5nrunokJyXPq1nIM8qd5VnE_92fJISzDA";
    var data = {
        "entry.170423798": uid,
        "entry.797986661": ct,
        "entry.873473661": block_n,
        "entry.834586908": trial_n,
        "entry.658891585": width,
        "entry.540029752": eww_ratio,
        "entry.1814923680": density,
        "entry.565547842": amplitude,
        "entry.1776692703": movement_time,
        "entry.1672558845": errors
    };
    var params = [];
    for (key in data) {
